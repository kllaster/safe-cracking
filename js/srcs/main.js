	...
	$(document).on("click", ".btn-try, .btn-try-auto", function() {
		...
		if (auto === true)
		{
			...
			let h_interval = setInterval(function () {
				ajax_attempt(robber, pin, safe_box, auto, item, h_interval);
			}, 1000);
		}
		else
			ajax_attempt(robber, pin, safe_box, auto, item);
	});

	function ajax_attempt(robber, pin, safe_box, auto, item, h_interval = null) {
		$.ajax({
			type: "GET",
			dataType: "text",
			url: "/app/ajax.php",
			...
			success: function(response) {
					...
					if (!!response.lock)
					{
						...
						let lock_div = $("#lock_"+robber);
						if (lock_div.length > 0)
							lock_div.html("The safe is blocked: <span>"+response.lock+" s.</span>");
						else
							item.children('.objects_try__item_row')
								.append("<div class=\"objects_try__item_lock\" id=\"lock_"+robber+"\">The safe is blocked: <span>"+response.lock+" s.</span></div>");
					}
					if (!!response.pin)
					{
						if (!!window.result)
						...
						$('.log_attempt').prepend("<div class=\"log_attempt__item\">\n" +
							"<div>Robber #"+response.robber+"</div>\n" +
			...
			success: function(response) {
				...
				let html = "<div class=\"objects_try__item\" id=\"objects_try__item"+response.key+"\">\n" +
							"<div class=\"objects_try__item_row\">\n" +
								"<div class=\"objects_try__item_name\">Robber #"+response.key+"</div>\n" +
